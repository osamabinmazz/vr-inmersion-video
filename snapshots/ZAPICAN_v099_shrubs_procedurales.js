...
// Mejora de calidad de texturas: el moteado ya no son 260 elipses al azar
// (makeFoliageMap, arriba, se deja sin usar por si hace falta volver atrás),
// sino una composición de la propia foto de hoja "ancha" ya aprobada del
// proyecto (varias copias rotadas/escaladas, desaturada a gris), generada
// una sola vez con gen_foliage_tex.mjs y guardada como PNG estático — mismo
// criterio que el resto de las texturas del proyecto (archivo en disco, no
// canvas en tiempo de ejecución). El bump sigue siendo procedural: alcanza
// como variación fina de superficie y no depende del follaje real.
const sharedFoliageMap = texLoader.load("/assets/textures/foliage_mottle/mottle_ancha.png");
sharedFoliageMap.wrapS = sharedFoliageMap.wrapT = THREE.RepeatWrapping;
sharedFoliageMap.repeat.set(3, 3);
sharedFoliageMap.colorSpace = THREE.SRGBColorSpace;
...
// --- Modelos 3D reales (opcional) -----------------------------------------
// A pedido explícito: el techo de calidad de la geometría procedural (ver
// informe de Fase 7) no llega a "casi hiperrealista". Si se coloca un
// archivo .glb en /assets/models/<especie>.glb, esta función lo carga y
// reemplaza la malla procedural del animal por el modelo real, SIN tocar
// la lógica de comportamiento/posición (que sigue moviendo el group entero,
// como siempre). Si el archivo no existe todavía, el fetch falla en
// silencio y se ve la fauna procedural de siempre — el sitio nunca se
// rompe mientras se consiguen los modelos.
const gltfLoader = new GLTFLoader();
const faunaMixers = [];
// Varios individuos de la misma especie (p.ej. 3 carpinchos) apuntan al
// mismo archivo .glb: se descarga y parsea UNA sola vez por ruta (Map de
// promesas) y cada individuo clona el resultado con SkeletonUtils (soporta
// huesos/skin, a diferencia de Object3D.clone()) — evita 3 fetches+parses
// del mismo archivo.
const modelLoadCache = new Map();
function tryUpgradeToModel(group, path, opts = {}) {
  if (!modelLoadCache.has(path)) {
    modelLoadCache.set(
      path,
      new Promise((resolve, reject) => gltfLoader.load(path, resolve, undefined, reject))
    );
  }
  modelLoadCache.get(path).then(
    (gltf) => {
      ...
      // Se OCULTA la geometría procedural en vez de borrarla: si el modelo
      // real tiene un problema, alcanza con volver a mostrar los hijos
      // originales sin tener que reconstruir el animal desde cero.
      for (const child of [...group.children]) child.visible = false;
      group.add(model);
      group.userData.realModel = model;
      // Gancho opcional para detalles puntuales por especie (ej. el copete
      // del tero) que no tiene sentido meter en esta función genérica.
      if (opts.onModel) opts.onModel(model);
      if (gltf.animations && gltf.animations.length) {
        const mixer = new THREE.AnimationMixer(model);
        // Nombre de clip desconocido de antemano (depende de quién exportó
        // el modelo): se prioriza algo que suene a reposo/caminata y, si no
        // hay ninguno reconocible, se usa el primer clip disponible.
        const clip =
          gltf.animations.find((c) => /idle|rest|stand|quiet|reposo/i.test(c.name)) ||
          gltf.animations[0];
        const action = mixer.clipAction(clip);
        action.play();
        // Desincroniza el arranque entre individuos (si no, los 3 carpinchos
        // bostezarían exactamente al mismo tiempo, en el mismo frame).
        mixer.setTime(faunaRng() * clip.duration);
        faunaMixers.push(mixer);
        group.userData.mixer = mixer;
      }
    },
    () => {
      /* todavía no hay modelo en esa ruta: se sigue viendo la fauna
         procedural, sin ensuciar la consola con un error esperado. */
    }
  );
}
...
// Registro compartido de "hogares" de fauna ya colocada (todas las
// especies), para poder espaciarlas ENTRE SÍ sin tocar cómo se agrupan
// los individuos de una misma especie (eso sigue siendo cosa de cada
// scatterPositions/waterOutlinePoint). Pedido explícito: "redistribuí los
// animales más naturalmente, no tan cercanos entre ellos" -- el problema
// real era que dos especies distintas podían caer cerca por coincidencia,
// ya que cada una se ubicaba sin saber de las demás.
const faunaHomes = [];
function scatterPositionsSpaced(count, rMin, rMax, minDistFromPOI, minDistFromOtherSpecies, species) {
  const result = [];
  let guard = 0;
  while (result.length < count && guard < count * 60) {
    guard++;
    const [candidate] = scatterPositions(1, rMin, rMax, minDistFromPOI);
    if (!candidate) continue;
    const [cx, cz] = candidate;
    const tooCloseToOtherSpecies = faunaHomes.some(
      (h) => h.species !== species && Math.hypot(h.x - cx, h.z - cz) < minDistFromOtherSpecies
    );
    if (!tooCloseToOtherSpecies) {
      result.push(candidate);
      faunaHomes.push({ x: cx, z: cz, species });
    }
  }
  return result;
}
...
// El carpincho real vive en grupos familiares -- casi nunca solo ni de a 2.
// Subido de 2 a 3 (pedido: "duplicar aquellos que andan en pareja o en
// manada").
...
for (let i = 0; i < CAPYBARA_COUNT; i++) {
  ...
  faunaHomes.push({ x, z, species: "capybara" });
  ...
  scene.add(capy);
  capybaras.push(capy);
  // Escala/rotación en 1 por defecto: se ajustan a ojo en cuanto llegue el
  // archivo real (cada exportador usa su propia convención de tamaño/eje).
  tryUpgradeToModel(capy, "/assets/models/capybara.glb", { scale: 1, rotationY: 0 });
}
...
// El ñandú real anda en grupos chicos fuera de temporada de cría. Subido de
// 2 a 3 (pedido: "duplicar aquellos que andan en pareja o en manada").
const RHEA_COUNT = 3;
const rheas = [];
// lejos del agua y de los marcadores: pastorean en la llanura abierta.
// scatterPositionsSpaced (no scatterPositions a secas) para que no termine
// pisándole el terreno a capybaras/venados por pura coincidencia.
const rheaPositions = scatterPositionsSpaced(RHEA_COUNT, 6, 12, 3.0, 4.5, "rhea");
rheaPositions.forEach(([x, z], i) => {
  ...
  scene.add(rhea);
  rheas.push(rhea);
  // Este modelo puntual es un avestruz (Struthio camelus), no un ñandú --
  // única opción gratis encontrada (ver informe de Fase 11); mide ~2,18 m
  // de alto tal cual viene, tamaño real de avestruz. El ñandú real mide
  // ~1,5-1,7 m: scale:0.72 lo acerca al tamaño correcto de la especie.
  tryUpgradeToModel(rhea, "/assets/models/rhea.glb", { scale: 0.72, rotationY: 0 });
});

// --- Nido (detalle ambiental cerca del ñandú hero) ------------------------
// Modelo real (no procedural), escaneo 3D con huevos, licencia CC0 (ver
// CREDITS_MODELOS_3D.txt). Es en realidad un nido de pato barcino (Anas
// zonorhyncha), no de ñandú -- no se encontró ningún escaneo de nido de
// ñandú gratis; se usa igual como detalle ambiental junto al ñandú hero por
// afinidad temática (pedido: "colócalo cerca del nandu"). Estático, sin
// comportamiento ni animación: una sola carga directa con GLTFLoader (no
// tryUpgradeToModel, que es el mecanismo de reemplazo de fauna procedural).
if (rheaPositions[0]) {
  const [nestHomeX, nestHomeZ] = rheaPositions[0];
  const nestAngle = rng() * Math.PI * 2;
  const nestDist = 1.3 + rng() * 0.4;
  const nestX = nestHomeX + Math.cos(nestAngle) * nestDist;
  const nestZ = nestHomeZ + Math.sin(nestAngle) * nestDist;
  gltfLoader.load(
    "/assets/models/nido.glb",
    (gltf) => {
      const nest = gltf.scene;
      nest.position.set(nestX, groundY(nestX, nestZ), nestZ);
      nest.rotation.y = rng() * Math.PI * 2;
      nest.traverse((o) => {
        if (o.isMesh) {
          o.castShadow = true;
          o.receiveShadow = true;
    ...
    undefined,
    () => {
      // Silencioso: si no está el archivo, sencillamente no aparece el nido
      // (mismo criterio de fallback que tryUpgradeToModel).
    }
  );
}
...
// El tero real es territorial en pareja -- subido de 3 a 4 (dos parejas) a
// pedido ("duplicar aquellos que andan en pareja o en manada").
const TERO_COUNT = 4;
const teros = [];
const teroPositions = scatterPositionsSpaced(TERO_COUNT, 3.5, 11, 1.6, 3.5, "tero");
teroPositions.forEach(([x, z], i) => {
  ...
  scene.add(tero);
  teros.push(tero);
  // Este modelo puntual es un playerito/andarríos (Actitis hypoleucos), no un
  // tero -- único gratis de calidad adecuada encontrado. Adaptado a mano
  // (ver CREDITS_MODELOS_3D.txt): patas alargadas, cuerpo redondeado, cuello
  // marcado y recoloreado a patrón de tero (pecho/garganta negro, vientre
  // blanco, cabeza gris, patas y pico rojizos con punta oscura). Mide ~1,04 m
  // tal cual viene (tamaño de playerito muy agrandado); el tero real mide
  // ~0,32-0,38 m: scale:0.31 lo lleva al tamaño correcto de la especie.
  tryUpgradeToModel(tero, "/assets/models/tero.glb", {
    scale: 0.31,
    rotationY: 0,
    // Copete fino (pedido explícito): el modelo base no lo trae, se agrega
    // como geometría procedural aparte, en el espacio local sin escalar del
    // modelo (hereda el scale:0.31 de arriba al ser hijo suyo).
    onModel: (model) => {
      const crestMat = new THREE.MeshStandardMaterial({ color: 0x161616, roughness: 0.85 });
      const crest = new THREE.Mesh(new THREE.ConeGeometry(0.014, 0.11, 6), crestMat);
      crest.position.set(0, 1.02, 0.33);
      crest.rotation.x = -Math.PI * 0.3;
...
// Primera especie nueva agregada más allá de las tres de la Fase 6 (pedido
// del usuario: proyecto educativo, "para que vean cómo eran flora y fauna").
// Herbívoro de pastizal, comportamiento tranquilo pero alerta -- mismo
// patrón de máquina de estados que el carpincho (REST/EAT/LOOK), pero sin
// nada de agua: pastorea en la llanura abierta, como el ñandú.
const venadoMat = new THREE.MeshStandardMaterial({ color: 0xa8663c, roughness: 0.85, flatShading: true, map: furTex, bumpMap: furBump, bumpScale: 0.01 });
const venadoBellyMat = new THREE.MeshStandardMaterial({ color: 0xe4d3b8, roughness: 0.85, flatShading: true, map: furTex.clone(), bumpMap: furBump.clone(), bumpScale: 0.01 });
const venadoDarkMat = new THREE.MeshStandardMaterial({ color: 0x2b2119, roughness: 0.7, flatShading: true });

function makeVenado(withAntlers) {
  const group = new THREE.Group();

  const body = new THREE.Mesh(new THREE.CapsuleGeometry(0.155, 0.5, 6, 12), venadoMat);
  body.rotation.z = Math.PI / 2;
  body.position.y = 0.52;
  body.castShadow = true;
  group.add(body);

  // vientre y garganta más claros: rasgo real del venado de campo, y ayuda
  // a distinguirlo del zorro/otro herbívoro de silueta parecida a lo lejos
  const belly = new THREE.Mesh(new THREE.CapsuleGeometry(0.09, 0.42, 5, 8), venadoBellyMat);
  belly.rotation.z = Math.PI / 2;
  belly.position.set(0, 0.4, 0);
  group.add(belly);

  // cuello y cabeza en su propio pivote, para bajar la cabeza al pastar
  const neckPivot = new THREE.Group();
  neckPivot.position.set(0.24, 0.66, 0);
  group.add(neckPivot);

  const neckGeo = new THREE.CylinderGeometry(0.05, 0.075, 0.32, 7);
  neckGeo.translate(0, 0.16, 0);
  const neck = new THREE.Mesh(neckGeo, venadoMat);
  neck.rotation.z = -0.55;
  neck.castShadow = true;
  neckPivot.add(neck);

  const throat = new THREE.Mesh(new THREE.SphereGeometry(0.045, 8, 6), venadoBellyMat);
  throat.position.set(0.1, 0.2, 0);
  neckPivot.add(throat);

  const head = new THREE.Mesh(new THREE.SphereGeometry(0.075, 10, 8), venadoMat);
  head.position.set(0.24, 0.32, 0);
  head.scale.set(1.35, 0.95, 0.85);
  head.castShadow = true;
  neckPivot.add(head);

  const snout = new THREE.Mesh(new THREE.ConeGeometry(0.035, 0.09, 8), venadoDarkMat);
  snout.rotation.z = -Math.PI / 2;
  snout.position.set(0.36, 0.29, 0);
  neckPivot.add(snout);

  // orejas grandes y erguidas: el rasgo más reconocible de un venado a
  // distancia, junto con el pelaje bicolor
  for (const side of [-1, 1]) {
    const ear = new THREE.Mesh(new THREE.ConeGeometry(0.035, 0.13, 6), venadoMat);
    ear.rotation.set(0, 0, side * 0.55);
    ear.position.set(0.24, 0.42, side * 0.06);
    group.userData.ears = group.userData.ears || [];
    group.userData.ears.push(ear);
    neckPivot.add(ear);
  }

  // astas chicas y bifurcadas: solo el individuo "macho" (hero, i===0) las
  // lleva -- las hembras de venado de campo no tienen, buen detalle
  // educativo de identificación de la especie sin costo extra de sistema.
  if (withAntlers) {
    for (const side of [-1, 1]) {
      const antlerBase = new THREE.Mesh(new THREE.CylinderGeometry(0.008, 0.01, 0.09, 5), venadoDarkMat);
      antlerBase.position.set(0.22, 0.47, side * 0.04);
      antlerBase.rotation.z = side * 0.18;
      neckPivot.add(antlerBase);
      const antlerFork = new THREE.Mesh(new THREE.CylinderGeometry(0.006, 0.008, 0.06, 5), venadoDarkMat);
      antlerFork.position.set(0.235, 0.535, side * 0.065);
      antlerFork.rotation.z = side * 0.6;
      neckPivot.add(antlerFork);
    }
  }

  // cola chica: el venado de campo no tiene la cola-bandera de otros
  // cérvidos, así que se deja pequeña y pegada al cuerpo
  const tail = new THREE.Mesh(new THREE.ConeGeometry(0.03, 0.09, 6), venadoBellyMat);
  tail.rotation.x = Math.PI;
  tail.position.set(-0.32, 0.5, 0);
  group.add(tail);

  // patas largas y finas, típicas de un corredor de pastizal abierto
  const legs = [];
  for (const [lx, lz] of [
    [0.16, 0.1],
    [0.16, -0.1],
    ...
    const leg = new THREE.Group();
    const upperGeo = new THREE.CylinderGeometry(0.032, 0.024, 0.26, 6);
    upperGeo.translate(0, -0.13, 0);
    const upper = new THREE.Mesh(upperGeo, venadoMat);
    upper.castShadow = true;
    leg.add(upper);
    const lowerGeo = new THREE.CylinderGeometry(0.02, 0.014, 0.24, 6);
    lowerGeo.translate(0, -0.12, 0);
    const lower = new THREE.Mesh(lowerGeo, venadoMat);
    lower.position.y = -0.26;
    leg.add(lower);
    const hoof = new THREE.Mesh(new THREE.ConeGeometry(0.018, 0.04, 6), venadoDarkMat);
    hoof.position.y = -0.5;
    leg.add(hoof);
...
const VENADO_STATES = {
  REST: { duration: [4, 9], next: ["EAT", "LOOK_UP", "SHORT_WALK"] },
  EAT: { duration: [3, 7], next: ["REST", "LOOK_UP", "EAT"], weight: (n) => (n === "EAT" ? 1.3 : 1) },
  LOOK_UP: { duration: [1.5, 3.5], next: ["REST", "EAT", "ALERT"] },
  ALERT: { duration: [1, 2.2], next: ["SHORT_WALK", "REST"] },
  SHORT_WALK: { duration: [2, 4], next: ["REST", "EAT"] },
};
// El venado de campo real se ve en grupos chicos, no solo de a 2. Subido de
// 2 a 3 (pedido: "duplicar aquellos que andan en pareja o en manada").
const VENADO_COUNT = 3;
const venados = [];
// llanura abierta, igual criterio que el ñandú pero un poco más lejos del
// agua/personajes -- el venado real evita el borde húmedo del humedal.
const venadoPositions = scatterPositionsSpaced(VENADO_COUNT, 7, 14, 3.5, 4.5, "venado");
venadoPositions.forEach(([x, z], i) => {
  const venado = makeVenado(i === 0);
  const y = groundY(x, z);
  venado.position.set(x, y, z);
  const heading = rng() * Math.PI * 2;
  venado.rotation.y = heading;
  Object.assign(venado.userData, {
    tier: i === 0 ? "HERO" : "SECONDARY",
    grazePhase: faunaRng() * Math.PI * 2,
    stepPhase: faunaRng() * Math.PI * 2,
    home: [x, z],
    ...
    heading,
    fromX: x, fromZ: z, toX: x, toZ: z, walkStart: 0,
    sm: makeStateMachine(VENADO_STATES, faunaRng, -faunaRng() * 6),
  });
  scene.add(venado);
  venados.push(venado);
  // Modelo real de pighunt3r15 (CC-BY-4.0 -- reemplaza al de WildMesh 3D,
  // CC-BY-NC-4.0, ver CREDITS): mejor proporcionado, y sin restricción
  // no-comercial. Sin rig ni animación (a diferencia del anterior), por eso
  // no hace falta AnimationMixer acá -- mismo caso que ñandú/tero/mulita.
  // El archivo trae las astas SOLDADAS AL MISMO MESH que el cuerpo; se
  // separaron en una segunda malla aparte (ver informe de fase) para poder
  // ocultarlas en las hembras/individuos secundarios, igual criterio que ya
  // usaba el venado procedural (astas solo en el hero).
  tryUpgradeToModel(venado, "/assets/models/venado.glb", {
    scale: 0.24,
    rotationY: 0,
    onModel: (model) => {
      if (i !== 0) {
...
// Segunda especie nueva agregada más allá de las tres de la Fase 6 (después
// del venado, Fase 10): armadillo chico de campo, solitario y de hábitos
// mayormente crepusculares -- a diferencia del carpincho/ñandú/venado, NO se
// duplica como pareja/manada (la especie real no forma grupos), aunque se
// ponen 2 individuos para que haya chance real de verlo en el recorrido.
// Geometría procedural mínima (sin patas articuladas ni orejas independientes,
// a diferencia del venado/ñandú): el modelo real ya llegó con muy buena
// calidad desde el principio, así que el fallback solo necesita existir para
// el caso de que el archivo no cargue, no para verse bien por su cuenta.
const mulitaMat = new THREE.MeshStandardMaterial({ color: 0x9c8f75, roughness: 0.9, flatShading: true, map: furTex.clone(), bumpMap: furBump.clone(), bumpScale: 0.008 });
const mulitaDarkMat = new THREE.MeshStandardMaterial({ color: 0x4a4133, roughness: 0.85, flatShading: true });

function makeMulita() {
  const group = new THREE.Group();

  const shell = new THREE.Mesh(new THREE.CapsuleGeometry(0.1, 0.26, 5, 10), mulitaMat);
  ...
  headPivot.position.set(0.17, 0.12, 0);
  group.add(headPivot);

  const snout = new THREE.Mesh(new THREE.ConeGeometry(0.035, 0.15, 7), mulitaDarkMat);
  snout.rotation.z = -Math.PI / 2 + 0.25;
  snout.position.set(0.08, -0.02, 0);
  snout.castShadow = true;
  headPivot.add(snout);

  const tail = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.03, 0.22, 6), mulitaDarkMat);
  tail.rotation.z = Math.PI / 2 - 0.3;
  tail.position.set(-0.24, 0.09, 0);
  group.add(tail);

  // patas cortas sin articular: apenas se ven bajo el caparazón, no vale la
  // pena un sistema de piernas completo para el fallback procedural.
  for (const [lx, lz] of [
    [0.09, 0.07], [0.09, -0.07], [-0.09, 0.07], [-0.09, -0.07],
  ]) {
    const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.018, 0.016, 0.09, 5), mulitaDarkMat);
...
const MULITA_STATES = {
  FORAGE: { duration: [3, 7], next: ["DIG", "SHORT_WALK", "ALERT"] },
  DIG: { duration: [2, 5], next: ["FORAGE", "SHORT_WALK"] },
  ALERT: { duration: [1, 2], next: ["FORAGE", "SHORT_WALK"] },
  SHORT_WALK: { duration: [1.5, 3], next: ["FORAGE", "DIG"] },
};
// Especie solitaria real: no se "duplica en pareja/manada" como las demás
// (no vendría al caso), pero se ponen 2 para más chance de encontrarlo.
const MULITA_COUNT = 2;
const mulitas = [];
// llanura abierta con algo de cobertura, igual criterio de espaciado que el
// venado/ñandú para que no termine pisándoles el terreno por coincidencia.
const mulitaPositions = scatterPositionsSpaced(MULITA_COUNT, 5, 12, 3.0, 4.0, "mulita");
mulitaPositions.forEach(([x, z], i) => {
  const mulita = makeMulita();
  const y = groundY(x, z);
  mulita.position.set(x, y, z);
  const heading = rng() * Math.PI * 2;
  mulita.rotation.y = heading;
  Object.assign(mulita.userData, {
    tier: i === 0 ? "SECONDARY" : "AMBIENT",
    forgePhase: faunaRng() * Math.PI * 2,
    stepPhase: faunaRng() * Math.PI * 2,
    home: [x, z],
    roam: 2.0,
    heading,
    fromX: x, fromZ: z, toX: x, toZ: z, walkStart: 0,
    sm: makeStateMachine(MULITA_STATES, faunaRng, -faunaRng() * 6),
  });
  scene.add(mulita);
  mulitas.push(mulita);
  // Modelo real de Nyilonelycompany (CC-BY-NC-4.0, ver CREDITS): mide ~1,42 m
  // de largo (hocico a punta de cola) tal cual viene, tamaño muy agrandado.
  // La mulita real mide ~0,40-0,55 m total: scale:0.35 la acerca al tamaño
  // correcto de la especie.
  tryUpgradeToModel(mulita, "/assets/models/mulita.glb", { scale: 0.35, rotationY: 0 });
});

// --- Rocas dispersas (detalle de terreno) ----------------------------------
// Prop inerte, sin comportamiento ni estado: a diferencia de la fauna, no
// hace falta clonar N veces con GLTFLoader -- un solo InstancedMesh comparte
// una única geometría/textura entre todas las copias, mucho más barato.
// Modelo real: "006-rock-lp" de waffen2009 (CC-BY-4.0, ver CREDITS), 210
// triángulos. El archivo mide ~0,45 m de ancho con scale:1 tal cual viene
// (sin bug de unidades, solo modelado a una escala arbitraria) -- se usa esa
// medida como referencia de "roca mediana" y se varía por instancia.
const ROCK_COUNT = 30;
gltfLoader.load(
  ...
  (gltf) => {
    ...
    gltf.scene.traverse((o) => {
      if (o.isMesh && !rockMesh) rockMesh = o;
    });
    if (!rockMesh) return;
    const instanced = new THREE.InstancedMesh(rockMesh.geometry, rockMesh.material, ROCK_COUNT);
    instanced.castShadow = true;
    instanced.receiveShadow = true;
    const rockPositions = scatterPositions(ROCK_COUNT, 2, 15, 1.2);
    const dummy = new THREE.Object3D();
    rockPositions.forEach(([x, z], i) => {
      const y = groundY(x, z);
      // 0,35x a 1,65x del tamaño de referencia (~0,45m): de piedras chicas a
      // algún que otro pedrón, variedad natural sin que domine la escena.
      const scaleFactor = (0.35 + rng() * 1.3) * 0.001;
      dummy.position.set(x, y, z);
  ...
  undefined,
  () => {
    // silencioso: si no está el archivo, sencillamente no aparecen rocas
    // (mismo criterio de fallback que tryUpgradeToModel/el nido).
  }
);

// --- Matas de pasto real (detalle suplementario, cerca/media distancia) ----
// A pedido: "integrar los chunks de pasto como cobertura suplementaria,
// usando instancing/LOD, priorizando el rendimiento VR, sin reemplazar el
// pasto procedural existente". El archivo original ("Simple grass chunks",
// 3dhdscan, CC-BY-4.0) trae parches ya escaneados/pre-dispersos de 18k-52k
// triángulos -- demasiado pesados para duplicar. La decisión de LOD fue
// justamente NO usarlos: se extrajeron solo las 3 variantes de card más
// livianas del mismo archivo (7-12 triángulos c/u, ya pensadas como
// "instancia individual" en vez de parche completo), y esas sí se
// instancian varias veces. Reemplazo del material BLEND original por MASK
// (recorte con alphaCutoff) para que las sombras salgan con la silueta real
// de la hoja en vez de un rectángulo sólido.
const GRASS_TUFT_MESHES = ["r12_a", "r12_b", "r12_c"];
const GRASS_TUFT_COUNT_EACH = 60;
gltfLoader.load(
  "/assets/models/grass_tufts.glb",
  (gltf) => {
    const dummy = new THREE.Object3D();
    let meshIndex = 0;
    gltf.scene.traverse((o) => {
      if (!o.isMesh) return;
      meshIndex++;
      const instanced = new THREE.InstancedMesh(o.geometry, o.material, GRASS_TUFT_COUNT_EACH);
      instanced.castShadow = true;
      instanced.receiveShadow = false;
      // cerca/media distancia (radio chico): cobertura suplementaria puntual,
      // no una alfombra general -- el pasto procedural ya cubre el resto.
      const positions = scatterPositions(GRASS_TUFT_COUNT_EACH, 1.5, 9, 1.0);
      positions.forEach(([x, z], i) => {
        const y = groundY(x, z);
        const scaleFactor = (0.7 + rng() * 0.6) * 0.0011;
        dummy.position.set(x, y, z);
        dummy.rotation.set(0, rng() * Math.PI * 2, 0);
        dummy.scale.setScalar(scaleFactor);
        dummy.updateMatrix();
  ...
  undefined,
  () => {
    // silencioso: si no está el archivo, sencillamente no aparecen estas
    // matas suplementarias (el pasto procedural sigue intacto igual).
  }
);

// --- Fase 18: especies placeholder del pack "4K PBR" (trimesh) ------------
// 13 especies pendientes (11 fauna + 2 flora) integradas a pedido explícito
// del usuario ("integrar todas las especies pendientes ahora como
// placeholders"), usando el pack de modelos que llegó en el lote grande de
// archivos. IMPORTANTE (ver CREDITS_MODELOS_3D.txt): son mallas generadas
// por script (trimesh), NO escaneos reales -- varias comparten literalmente
// la misma plantilla genérica de "cuadrúpedo" o "ave" recoloreada, y no
// tienen coordenadas UV, así que el set de texturas 4K que acompaña al pack
// no se pudo aplicar (haría falta un unwrap manual en Blender, no disponible
// en este entorno): el color viene solo del vertex color ya horneado en cada
// archivo (ver apply_placeholder_material.mjs, que arma el material PBR
// liso -- roughness fijo por tipo de superficie -- y deja el vertex color
// intacto). Sin pose de pie creíble (patas "abiertas" en vez de apoyadas):
// en vez de fingir una caminata que quedaría mal, solo se les da un balanceo
// de reposo sutil. Reemplazar especie por especie cuando haya un modelo
// real, igual que se hizo con tero/venado/mulita en fases anteriores.
const placeholderFauna = [];
// `tint` (opcional, color hex): varias de estas 13 especies comparten
// LITERALMENTE la misma malla/vertex-color de origen (ver README del pack,
// Fase 18) -- p.ej. zorro y guazubira salen ambos del mismo tono tostado.
// Un multiply de color sobre el material (sin tocar el vertex color en sí)
// alcanza para diferenciarlas a simple vista sin depender de un modelo
// nuevo.
function addPlaceholderLand(name, path, { scale, yOffset = 0, count, rMin, rMax, minDistFromPOI = 3, minDistFromOther = 3, tint }) {
  const positions = scatterPositionsSpaced(count, rMin, rMax, minDistFromPOI, minDistFromOther, name);
  positions.forEach(([x, z]) => {
    const group = new THREE.Group();
    group.position.set(x, groundY(x, z), z);
    group.rotation.y = rng() * Math.PI * 2;
    group.userData = { swayPhase: faunaRng() * Math.PI * 2, swaySpeed: 0.5 + faunaRng() * 0.3 };
    scene.add(group);
    placeholderFauna.push(group);
    tryUpgradeToModel(group, path, {
      scale,
      yOffset,
      onModel: tint
        ? (model) => model.traverse((o) => { if (o.isMesh) o.material.color.setHex(tint); })
        : undefined,
    });
  });
}
// Especies de orilla (nutria, lobo marino, chajá, garza, tortuga, sapito):
// se ubican con waterOutlinePoint como el carpincho, en vez de scatter
// general, para que queden junto al agua en vez de perdidas en la llanura.
function addPlaceholderWaterEdge(name, path, { scale, yOffset = 0, count, scaleMin, scaleMax, minDistFromOther = 2.5, tint }) {
  for (let i = 0; i < count; i++) {
    let x, z, tries = 0;
    do {
      const angle = faunaRng() * Math.PI * 2;
      const rScale = scaleMin + faunaRng() * (scaleMax - scaleMin);
      [x, z] = waterOutlinePoint(angle, rScale);
      tries++;
    } while (
      faunaHomes.some((h) => Math.hypot(h.x - x, h.z - z) < minDistFromOther) &&
      tries < 40
    );
    faunaHomes.push({ x, z, species: name });
    const group = new THREE.Group();
    group.position.set(x, groundY(x, z), z);
    group.rotation.y = Math.atan2(WATER_CENTER[0] - x, WATER_CENTER[1] - z) + Math.PI / 2;
    group.userData = { swayPhase: faunaRng() * Math.PI * 2, swaySpeed: 0.5 + faunaRng() * 0.3 };
    scene.add(group);
    placeholderFauna.push(group);
    tryUpgradeToModel(group, path, {
...
// Mamíferos (zorro de monte, gato montés, guazubira: solitarios de llanura;
// lobito de río y lobo marino: orilla). NOTA sobre el lobo marino: es una
// especie estrictamente costera/marina, no de laguna de agua dulce -- se
// incluye igual porque así lo pidió el usuario en el lote aprobado, pero es
// una licencia ecológica del proyecto, no un hábitat real para la especie
// (documentado también en CREDITS).
addPlaceholderLand("zorro", "/assets/models/zorro.glb", { scale: 0.333, count: 1, rMin: 6, rMax: 18 });
addPlaceholderLand("gato_montes", "/assets/models/gato_montes.glb", { scale: 0.288, count: 1, rMin: 6, rMax: 18 });
// tint: la plantilla de origen le da el mismo tostado que al zorro (ambos
// vienen del mismo molde genérico, ver Fase 18) -- un multiply gris-oliva
// la acerca al pelaje real de la guazubira y evita que se confundan a
// simple vista si aparecen cerca.
addPlaceholderLand("guazubira", "/assets/models/guazubira.glb", { scale: 0.532, count: 1, rMin: 6, rMax: 16, minDistFromOther: 4, tint: 0xa8b09a });
addPlaceholderWaterEdge("lobito", "/assets/models/lobito.glb", { scale: 0.357, count: 2, scaleMin: 0.96, scaleMax: 1.05 });
addPlaceholderWaterEdge("lobo_marino", "/assets/models/lobo_marino.glb", { scale: 0.864, yOffset: -0.095, count: 1, scaleMin: 0.95, scaleMax: 1.02 });

// Aves (chajá y garza mora: orilla; cardenal amarillo: pastizal/monte).
// Distribución: el cardenal mide ~19 cm real (scale 0.146) -- con el rMax
// original de 14 m quedaba tan chico en pantalla desde el punto de vista
// fijo que era prácticamente invisible; bajado a 9 m para que el tamaño
// real del animal sea perceptible sin acercarlo tanto como para chocar con
// otra fauna de la llanura.
addPlaceholderWaterEdge("chaja", "/assets/models/chaja.glb", { scale: 0.521, count: 2, scaleMin: 1.3, scaleMax: 1.6 });
addPlaceholderLand("cardenal", "/assets/models/cardenal.glb", { scale: 0.146, count: 2, rMin: 4, rMax: 9, minDistFromOther: 2.5 });
// tint: mismo molde de ave genérica que chajá -- un multiply azul-grisáceo
// se acerca más al plumaje real de la garza mora (Ardea cocoi) y la
// distingue del chajá si ambas quedan a la vista al mismo tiempo.
addPlaceholderWaterEdge("garza_mora", "/assets/models/garza_mora.glb", { scale: 0.498, count: 1, scaleMin: 1.0, scaleMax: 1.1, tint: 0x93a8ba });

// Reptiles y anfibios (lagarto overo: pastizal soleado; tortuga y sapito: orilla).
addPlaceholderLand("lagarto", "/assets/models/lagarto.glb", { scale: 0.427, yOffset: -0.0342, count: 1, rMin: 5, rMax: 14 });
addPlaceholderWaterEdge("tortuga", "/assets/models/tortuga.glb", { scale: 0.239, yOffset: -0.0263, count: 2, scaleMin: 1.0, scaleMax: 1.15 });
addPlaceholderWaterEdge("sapito", "/assets/models/sapito.glb", { scale: 0.0933, yOffset: -0.0056, count: 2, scaleMin: 1.1, scaleMax: 1.3, minDistFromOther: 1.5 });

// Guayabo del país y arazá (Fase 23): reemplaza el placeholder crudo de la
// Fase 18 -- al usuario no le gustó (con razón: un clúster de esferas
// idénticas generado por script, sin relación real con ninguna de las dos
// especies, pegado como un objeto ajeno al lado del monte real). Se
// reconstruyen con el MISMO sistema de lóbulos irregulares + corteza que ya
// usa el resto del monte nativo (espinillo/algarrobo/ceibo -- ver
// buildLobeMass más arriba), en vez de depender de otro modelo externo:
// encajan con el estilo del resto de la vegetación en lugar de destacar
// como un agregado tardío. Sin sistema de viento (son solo 4 ejemplares
// puntuales, no vale la pena el shader completo para tan pocos).
function buildShrubCanopyGeo(layout, detail, seed, color) {
  const geo = buildLobeMass(layout, detail, seed, { amount: 0.24, flatten: 0.5 });
  const pos = geo.attributes.position;
  const ao = geo.attributes.aAO;
  ...
  geo.deleteAttribute("aAO");
  geo.setAttribute("color", new THREE.BufferAttribute(col, 3));
  return geo;
}
function makeNativeShrub({ seed, colorA, colorB, trunkTint, accentColor, accentCount, accentRadius }) {
  const shrubRng = mulberry32(seed);
  const group = new THREE.Group();

  const trunkH = 0.5 + shrubRng() * 0.25;
  const trunkGeo = new THREE.CylinderGeometry(0.02, 0.04, trunkH, 7);
  trunkGeo.translate(0, trunkH / 2, 0);
  const trunk = new THREE.Mesh(trunkGeo, makeBarkMaterial(trunkTint));
  trunk.castShadow = true;
  group.add(trunk);

  // Lóbulo central más grande + 4-5 lóbulos secundarios alrededor, igual
  // criterio que las variantes de copa del monte (CANOPY_VARIANTS) pero a
  // escala de arbusto en vez de árbol.
  const lobeCount = 4 + (shrubRng() < 0.5 ? 0 : 1);
  const layout = [[0, 0.1, 0, 0.27]];
  for (let i = 0; i < lobeCount; i++) {
    const a = (i / lobeCount) * Math.PI * 2 + shrubRng() * 0.5;
    const off = 0.15 + shrubRng() * 0.09;
    const r = 0.19 + shrubRng() * 0.07;
    layout.push([Math.cos(a) * off, (shrubRng() - 0.35) * 0.14, Math.sin(a) * off, r]);
  }
  const inflated = inflateLobes(layout, 1.15);
  const color = new THREE.Color().lerpColors(new THREE.Color(colorA), new THREE.Color(colorB), shrubRng());
  const canopyGeo = buildShrubCanopyGeo(inflated, 2, seed, color);
  const canopy = new THREE.Mesh(canopyGeo, new THREE.MeshStandardMaterial({ vertexColors: true, roughness: 0.85 }));
  canopy.position.y = trunkH * 0.82;
  canopy.castShadow = true;
  canopy.receiveShadow = true;
  group.add(canopy);

  // Flores/frutos: bolitas chicas apoyadas sobre la superficie real de los
  // lóbulos (lobeSurface, la misma función que usa el follaje del monte
  // para no quedar flotando en los huecos entre lóbulos).
  if (accentCount) {
    const accentMat = new THREE.MeshStandardMaterial({ color: accentColor, roughness: 0.6 });
    const accentGeo = new THREE.SphereGeometry(accentRadius, 6, 5);
    const surf = new THREE.Vector3();
    for (let i = 0; i < accentCount; i++) {
      const theta = shrubRng() * Math.PI * 2;
      const phi = Math.acos(1 - shrubRng() * 1.2);
      const nx = Math.sin(phi) * Math.cos(theta);
      const ny = Math.cos(phi);
      const nz = Math.sin(phi) * Math.sin(theta);
      lobeSurface(inflated, nx, ny, nz, surf);
      const accent = new THREE.Mesh(accentGeo, accentMat);
      accent.position.set(surf.x, canopy.position.y + surf.y, surf.z);
...
// Guayabo del país (Acca sellowiana): follaje gris-plateado (el envés de la
// hoja es muy claro) y flor blanca vistosa de estambres rojos, comestible
// -- los puntos claros son esa flor, no fruto.
const guayaboPositions = scatterPositionsSpaced(2, 4, 14, 2, 3, "guayabo");
guayaboPositions.forEach(([x, z], i) => {
  const shrub = makeNativeShrub({
    seed: 810000 + i * 97,
    colorA: 0x8a9a7a,
    colorB: 0x6f8560,
    trunkTint: 0xb0a58e,
    accentColor: 0xf3ece0,
    accentCount: 10,
    accentRadius: 0.025,
  });
  shrub.position.set(x, groundY(x, z), z);
  shrub.rotation.y = rng() * Math.PI * 2;
  scene.add(shrub);
});

// Arazá (Psidium cattleianum, "guayaba fresa"): follaje verde oscuro
// brillante, fruto redondo rojo/granate al madurar.
const arazaPositions = scatterPositionsSpaced(2, 4, 14, 2, 3, "araza");
arazaPositions.forEach(([x, z], i) => {
  const shrub = makeNativeShrub({
    seed: 820000 + i * 97,
    colorA: 0x3f6b3c,
    colorB: 0x2c4a2a,
    trunkTint: 0x8d7358,
    accentColor: 0x9c2f35,
    accentCount: 8,
    accentRadius: 0.022,
  });
  shrub.position.set(x, groundY(x, z), z);
...
// Cobertura de suelo placeholder (pastizal nativo y pajonal de bañado, Tarea
// 42): a diferencia de las 13 especies de arriba, el .glb de origen no es UN
// espécimen sino UN solo matorral ya armado con 120 primitivas de "hoja"
// (mismo criterio que las matas de pasto de la Fase 17), así que se integra
// igual que grass_tufts.glb -- InstancedMesh, muchas copias como cobertura
// suplementaria, no como reemplazo del pasto procedural existente.
const pastizalPositions = scatterPositions(35, 3, 16, 1.5);
gltfLoader.load(
  "/assets/models/pastizal_nativo.glb",
  (gltf) => {
    let mesh = null;
    gltf.scene.traverse((o) => { if (o.isMesh && !mesh) mesh = o; });
    if (!mesh) return;
    const instanced = new THREE.InstancedMesh(mesh.geometry, mesh.material, pastizalPositions.length);
    instanced.castShadow = true;
    instanced.receiveShadow = true;
    const dummy = new THREE.Object3D();
    pastizalPositions.forEach(([x, z], i) => {
      const y = groundY(x, z);
      // Archivo mide ~2,4 m de lado tal cual viene; escala 0,167 lo lleva a
      // un matorral de pastizal nativo de ~0,4 m, con variación por copia.
      const scaleFactor = 0.167 * (0.8 + rng() * 0.5);
      dummy.position.set(x, y - 0.18 * scaleFactor, z);
      dummy.rotation.set(0, rng() * Math.PI * 2, 0);
      dummy.scale.setScalar(scaleFactor);
      dummy.updateMatrix();
      instanced.setMatrixAt(i, dummy.matrix);
...
// Pajonal de bañado: junto a la orilla (banda húmeda, radiusScale > 1 como
// el resto de la vegetación de borde), no en la llanura seca.
const PAJONAL_COUNT = 25;
gltfLoader.load(
  "/assets/models/pajonal_banado.glb",
  (gltf) => {
    let mesh = null;
    gltf.scene.traverse((o) => { if (o.isMesh && !mesh) mesh = o; });
    if (!mesh) return;
    const instanced = new THREE.InstancedMesh(mesh.geometry, mesh.material, PAJONAL_COUNT);
    instanced.castShadow = true;
    instanced.receiveShadow = true;
    const dummy = new THREE.Object3D();
    for (let i = 0; i < PAJONAL_COUNT; i++) {
      const a = rng() * Math.PI * 2;
      const rScale = 1.3 + rng() * 0.6;
      const [x, z] = waterOutlinePoint(a, rScale);
      const y = groundY(x, z);
      // Archivo mide ~2,1 m de lado tal cual viene; escala 0,285 lo lleva a
      // un pajonal de ~0,6 m, algo más grande que el pastizal seco.
      const scaleFactor = 0.285 * (0.8 + rng() * 0.5);
      dummy.position.set(x, y, z);
      dummy.rotation.set(0, rng() * Math.PI * 2, 0);
      dummy.scale.setScalar(scaleFactor);
      dummy.updateMatrix();
...
// --- Sauce llorón (Salix babylonica) — ejemplar ornamental aislado --------
// A diferencia de los sauces criollos de arriba (Salix humboldtiana, la
// especie NATIVA del monte ribereño, procedurales, inclinados sobre el
// agua), este es un modelo real de la especie ORNAMENTAL/introducida
// (asiática) que el usuario pidió sumar aparte -- sin tocar los 4 sauces
// criollos existentes ni su ubicación junto a la laguna. Se lo ubica lejos
// de la orilla, en la llanura abierta, para que lea como un ejemplar suelto
// plantado en el campo (uso real y común de esta especie en Uruguay) y no
// como parte de la galería nativa de la costa. Ver CREDITS_MODELOS_3D.txt.
const [sauceLloronX, sauceLloronZ] = scatterPositions(1, 12, 20, 5)[0] || [14, -10];
gltfLoader.load(
  "/assets/models/sauce_lloron.glb",
  (gltf) => {
    ...
    // El archivo ya viene en unidades reales (~8 m de alto, ~11-12 m de
    // copa) -- sin bug de escala, no hace falta factor de corrección.
    model.position.set(sauceLloronX, groundY(sauceLloronX, sauceLloronZ), sauceLloronZ);
...
// --- El Centinela: estatua que despierta y camina (Fase 20) ---------------
// Dos archivos del mismo personaje (ver CREDITS_MODELOS_3D.txt sobre su
// procedencia y las salvedades de precisión cultural): uno estático (pose de
// estatua, "The Sentinel") y otro con esqueleto Mixamo estándar sin clips de
// animación propios. El archivo con esqueleto traía las inverseBindMatrices
// corruptas/desalineadas -- el mesh crudo (sin skin) se ve perfecto pero con
// el skin aplicado salía deformado en una masa irreconocible. Se corrige en
// tiempo de carga con skeleton.calculateInverses()+pose() (recalcula las
// matrices a partir de la pose real de los huesos): confirmado con captura
// aislada que arma/piernas quedan independientes y correctas tras el fix.
// Mecánica pedida por el usuario: arranca como estatua; a los pocos segundos
// "despierta" -- la estatua desaparece y el modelo con esqueleto la
// reemplaza en el mismo lugar, camina unos metros hacia el interior del
// territorio con un ciclo de piernas/brazos simple (sin clip de Mixamo
// disponible, se anima a mano rotando los huesos por nombre) y se detiene.
// Transformación de un solo sentido -- no vuelve a ser estatua.
const WARRIOR_HOME = [-6, 2.5];
const WARRIOR_SCALE_STATUE = 1.7 / 1.253; // bbox real ~1,253 m alto -> 1,7 m humano
const WARRIOR_SCALE_RIGGED = 1.7 / 1.0; // bbox (ya con el fix) ~1,0 m alto -> 1,7 m
const WARRIOR_AWAKEN_AT = 20; // segundos desde el arranque del render loop
const WARRIOR_WALK_SPEED = 0.55; // m/s, paso lento y ceremonial
const WARRIOR_WALK_DURATION = 13; // s -- ~7 m de recorrido antes de detenerse
const WARRIOR_HEADING = Math.PI + 0.25; // hacia -Z (adentro del territorio, lejos del punto de vista), con leve desvío

let centinelaStatue = null;
let centinelaWarrior = null;
let centinelaBones = null;
let centinelaAwakened = false;

gltfLoader.load(
  "/assets/models/estatua_centinela.glb",
  (gltf) => {
    const model = gltf.scene;
    model.traverse((o) => {
      if (o.isMesh) {
    ...
    model.scale.setScalar(WARRIOR_SCALE_STATUE);
    model.position.set(WARRIOR_HOME[0], groundY(WARRIOR_HOME[0], WARRIOR_HOME[1]), WARRIOR_HOME[1]);
    // Mirando hacia el punto de vista, como corresponde a un centinela.
    model.rotation.y = Math.atan2(HERO_VIEW_POS.x - WARRIOR_HOME[0], HERO_VIEW_POS.z - WARRIOR_HOME[1]);
    scene.add(model);
    centinelaStatue = model;
  },
  undefined,
  () => {}
);
...
gltfLoader.load(
  "/assets/models/guerrero_animado.glb",
  (gltf) => {
    const model = gltf.scene;
    ...
    // Ver comentario de más arriba: arregla las inverseBindMatrices rotas
    // del archivo de origen.
    skinnedMesh.skeleton.calculateInverses();
    skinnedMesh.skeleton.pose();
    const bones = {};
    for (const bone of skinnedMesh.skeleton.bones) bones[bone.name] = bone;

    model.scale.setScalar(WARRIOR_SCALE_RIGGED);
    model.position.set(WARRIOR_HOME[0], groundY(WARRIOR_HOME[0], WARRIOR_HOME[1]), WARRIOR_HOME[1]);
    model.rotation.y = WARRIOR_HEADING;
    model.visible = false; // aparece recién cuando "despierta"
    scene.add(model);
    centinelaWarrior = model;
    centinelaBones = bones;
  },
  undefined,
  () => {}
);

// --- Segunda estatua decorativa (misma línea comercial que El Centinela) --
// A diferencia del anterior, este es solo decorativo -- sin mecánica de
// despertar/caminar (pedido explícito del usuario: "otro centinela más,
// decorativo"). Mismas salvedades de procedencia/licencia desconocida e
// iconografía genérica no-Charrúa, ver CREDITS_MODELOS_3D.txt. El archivo
// de origen pesaba 983.816 triángulos -- reducido a ~23.7k con gltf-transform
// simplify (ratio 0.02) antes de subirlo, la malla resultante es
// visualmente indistinguible del original a la distancia de esta escena.
const [centinela2X, centinela2Z] = scatterPositions(1, 8, 16, 4)[0] || [10, -8];
gltfLoader.load(
  "/assets/models/estatua_centinela2.glb",
  (gltf) => {
    const model = gltf.scene;
    model.traverse((o) => {
      if (o.isMesh) {
    ...
    // Bbox real ~0,995 m alto -> 1,7 m (altura humana adulta).
    model.scale.setScalar(1.7 / 0.995);
    model.position.set(centinela2X, groundY(centinela2X, centinela2Z), centinela2Z);
    model.rotation.y = rng() * Math.PI * 2;
    scene.add(model);
  },
  undefined,
...
const QC_CAMERAS = {
  ...
  QC_VENADO: { pos: [venados[0].position.x + 3.5, 2.2, venados[0].position.z + 2.8], look: [venados[0].position.x, 0.5, venados[0].position.z] },
...
renderer.setAnimationLoop((time) => {
  currentTime = time;
  perfTick(time);
  // Anima los clips propios de un modelo real cargado (tryUpgradeToModel),
  // si lo hay: independiente de la máquina de estados, que sigue moviendo
  // el group entero igual que con la fauna procedural.
  const dt = lastFrameMs ? Math.min(0.1, (time - lastFrameMs) / 1000) : 0;
  lastFrameMs = time;
  for (const mixer of faunaMixers) mixer.update(dt);
  ...
  // --- Venado de campo: HERO/SECONDARY_FAUNA en la llanura abierta --------
  // Mismo patrón de movimiento que el ñandú (roam alrededor de un "home"),
  // pero la cabeza baja a pastar (neck pivot) en vez de picotear, y las
  // orejas se paran en ALERT -- gesto reconocible de un herbívoro cauto.
  for (const venado of venados) {
    const ud = venado.userData;
    const transitioned = stepStateMachine(ud.sm, VENADO_STATES, t, faunaRng);
    if (transitioned && ud.sm.state === "SHORT_WALK") {
      ud.fromX = venado.position.x;
      ud.fromZ = venado.position.z;
      let tx = ud.home[0], tz = ud.home[1];
      for (let tries = 0; tries < 8; tries++) {
        const a = faunaRng() * Math.PI * 2;
        const r = faunaRng() * ud.roam;
    ...
    const since = t - ud.sm.since;
    const total = Math.max(0.001, ud.sm.until - ud.sm.since);
    const ease = Math.min(1, since / 0.4) * Math.min(1, (total - since) / 0.4);
    let neckTarget = -0.55; // reposo: cabeza erguida, ni pastando ni alerta
    if (ud.sm.state === "EAT") neckTarget = -0.55 + Math.max(0, ease) * (1.15 + 0.1 * Math.sin(t * 2 + ud.grazePhase));
    else if (ud.sm.state === "ALERT") neckTarget = -0.55 * (1 - Math.max(0, ease) * 0.6);
    venado.userData.neck.rotation.z = neckTarget;
    // orejas erguidas en ALERT/LOOK_UP, relajadas el resto del tiempo
    const earUp = ud.sm.state === "ALERT" || ud.sm.state === "LOOK_UP" ? Math.max(0, ease) : 0;
    for (const ear of venado.userData.ears || []) ear.rotation.x = -earUp * 0.35;

    if (ud.sm.state === "SHORT_WALK") {
      const p = Math.min(1, (t - ud.walkStart) / total);
      const x = THREE.MathUtils.lerp(ud.fromX, ud.toX, p);
      const z = THREE.MathUtils.lerp(ud.fromZ, ud.toZ, p);
      venado.position.set(x, groundY(x, z), z);
      const swing = p < 1 ? 1 : 0;
      venado.userData.legs.forEach((leg, i) => { leg.rotation.x = swing * Math.sin(t * 7 + i * Math.PI) * 0.35; });
    } else {
      venado.position.y = groundY(venado.position.x, venado.position.z) + 0.01 * Math.sin(t * 1.1 + ud.stepPhase);
      for (const leg of venado.userData.legs) leg.rotation.x = 0;
    }
    venado.rotation.y = ud.heading;
  }

  // --- Mulita: AMBIENT_FAUNA, especie solitaria de hábitos terrestres -----
  // Sin patas articuladas (ver makeMulita): la única animación es el hocico
  // metido en tierra al escarbar y levantado en ALERT, más el desplazamiento
  // corto entre puntos de forrajeo.
  for (const mulita of mulitas) {
    const ud = mulita.userData;
    const transitioned = stepStateMachine(ud.sm, MULITA_STATES, t, faunaRng);
    if (transitioned && ud.sm.state === "SHORT_WALK") {
      ud.fromX = mulita.position.x;
      ud.fromZ = mulita.position.z;
      let tx = ud.home[0], tz = ud.home[1];
      for (let tries = 0; tries < 8; tries++) {
        const a = faunaRng() * Math.PI * 2;
        const r = faunaRng() * ud.roam;
    ...
    const total = Math.max(0.001, ud.sm.until - ud.sm.since);
    const ease = Math.min(1, since / 0.4) * Math.min(1, (total - since) / 0.4);
    let noseTarget = 0; // reposo: hocico neutro
    if (ud.sm.state === "DIG") noseTarget = Math.max(0, ease) * 0.5;
    else if (ud.sm.state === "ALERT") noseTarget = -Math.max(0, ease) * 0.35;
    else if (ud.sm.state === "FORAGE") noseTarget = 0.12 * Math.sin(t * 1.6 + ud.forgePhase);
    if (mulita.userData.headPivot) {
      mulita.userData.headPivot.rotation.x = noseTarget;
      mulita.userData.headPivot.rotation.y = ud.sm.state === "FORAGE" ? 0.25 * Math.sin(t * 0.9 + ud.forgePhase) : 0;
    }
    ...
    if (ud.sm.state === "SHORT_WALK") {
      ...
      const x = THREE.MathUtils.lerp(ud.fromX, ud.toX, p);
      const z = THREE.MathUtils.lerp(ud.fromZ, ud.toZ, p);
      mulita.position.set(x, groundY(x, z), z);
    } else {
      mulita.position.y = groundY(mulita.position.x, mulita.position.z);
    }
    mulita.rotation.y = ud.heading;
  }

  // --- Fauna placeholder (Fase 18): sin pose de pie creíble (patas
  // "abiertas" en la geometría genérica) -- balanceo de reposo sutil nada
  // más, ver comentario junto a addPlaceholderLand/WaterEdge más arriba.
  for (const p of placeholderFauna) {
    const ud = p.userData;
    p.position.y = groundY(p.position.x, p.position.z) + 0.015 * Math.sin(t * ud.swaySpeed + ud.swayPhase);
  }

  // --- El Centinela: despertar + caminata (Fase 20) -----------------------
  if (!centinelaAwakened && centinelaStatue && centinelaWarrior && t > WARRIOR_AWAKEN_AT) {
    centinelaAwakened = true;
    centinelaStatue.visible = false;
    centinelaWarrior.visible = true;
    centinelaWarrior.userData.walkStart = t;
  }
  if (centinelaAwakened && centinelaWarrior && centinelaBones) {
    const since = t - centinelaWarrior.userData.walkStart;
    const walking = since < WARRIOR_WALK_DURATION;
    // Sin clip de Mixamo disponible en este archivo (ver comentario más
    // arriba): ciclo de piernas/brazos a mano, mismo criterio que el swing
    // de patas del ñandú en SHORT_WALK.
    const strideHz = 2.1;
    const swing = walking ? Math.sin(since * strideHz * Math.PI * 2) : 0;
    // Amplitud del balanceo de piernas/brazos: sube y baja de a poco (evita
    // que el ciclo arranque o corte de golpe a mitad de zancada). Esto NO
    // debe afectar la distancia caminada (más abajo) -- si se usara la misma
    // variable ahí, el cuerpo retrocedería hacia el punto de partida justo
    // cuando el balanceo se apaga al final.
    const swingAmp = walking ? Math.min(1, since / 1.0) * Math.min(1, (WARRIOR_WALK_DURATION - since) / 1.0) : 0;
    // El bind pose del archivo es un T-pose (brazos horizontales, ver
    // captura de diagnóstico). IMPORTANTE: el skin de este archivo (ver
    // comentario más arriba sobre las inverseBindMatrices rotas) tolera bien
    // la rotación de mixamorig(Left|Right)Arm en X (adelante/atrás, el eje
    // natural del vaivén al caminar) pero se ESTIRA en una tira larga y
    // deforme al rotar en Z (bajar el brazo hacia el costado) -- confirmado
    // con una grilla de comparación aislada, incluso a ángulos chicos. Por
    // eso el balanceo de brazos usa X nada más, y no se intenta bajar del
    // todo los brazos a los costados (esa rotación en Z está descartada).
    const IDLE_ARM_X = 0.5;
    centinelaBones.mixamorigLeftUpLeg.rotation.x = swing * 0.5 * swingAmp;
    centinelaBones.mixamorigRightUpLeg.rotation.x = -swing * 0.5 * swingAmp;
    centinelaBones.mixamorigLeftLeg.rotation.x = Math.max(0, -swing) * 0.7 * swingAmp;
    centinelaBones.mixamorigRightLeg.rotation.x = Math.max(0, swing) * 0.7 * swingAmp;
    centinelaBones.mixamorigLeftArm.rotation.x = -IDLE_ARM_X - swing * 0.35 * swingAmp;
    centinelaBones.mixamorigRightArm.rotation.x = -IDLE_ARM_X + swing * 0.35 * swingAmp;
    centinelaBones.mixamorigSpine.rotation.y = swing * 0.06 * swingAmp;

    if (walking) {
      const dx = Math.sin(WARRIOR_HEADING);
      const dz = Math.cos(WARRIOR_HEADING);
      const dist = since * WARRIOR_WALK_SPEED;
      const x = WARRIOR_HOME[0] + dx * dist;
      const z = WARRIOR_HOME[1] + dz * dist;
      centinelaWarrior.position.set(x, groundY(x, z), z);
    }
  }
