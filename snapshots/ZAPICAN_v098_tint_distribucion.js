...
// Flora: sin balanceo (no tiene el shader de viento del resto del follaje;
// agregárselo a una malla temporal de placeholder no vale la pena), solo
// ubicación.
function addPlaceholderFlora(name, path, { scale, yOffset = 0, count, rMin, rMax, minDistFromPOI = 2, minDistFromOther = 3 }) {
  const positions = scatterPositionsSpaced(count, rMin, rMax, minDistFromPOI, minDistFromOther, name);
  positions.forEach(([x, z]) => {
    const group = new THREE.Group();
    group.position.set(x, groundY(x, z), z);
    group.rotation.y = rng() * Math.PI * 2;
    scene.add(group);
    tryUpgradeToModel(group, path, { scale, yOffset });
  });
}
...
// Flora (guayabo del país y arazá: arbustos/árboles chicos de monte nativo).
addPlaceholderFlora("guayabo", "/assets/models/guayabo.glb", { scale: 0.864, count: 2, rMin: 4, rMax: 14 });
addPlaceholderFlora("araza", "/assets/models/araza.glb", { scale: 0.886, count: 2, rMin: 4, rMax: 14 });
