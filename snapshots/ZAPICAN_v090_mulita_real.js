...
venadoPositions.forEach(([x, z], i) => {
  ...
  scene.add(venado);
  venados.push(venado);
  // Este modelo puntual (WildMesh 3D) viene exportado en centímetros, no en
  // metros: sin corregir, medía ~184 m de alto (verificado con Box3 real).
  // scale:0.01 lo vuelve a un tamaño de venado real (~1,8 m).
  tryUpgradeToModel(venado, "/assets/models/venado.glb", { scale: 0.01, rotationY: 0 });
});
